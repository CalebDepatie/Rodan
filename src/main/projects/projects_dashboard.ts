...
ipcMain.handle('projects-get', async (e, req) => {
  try {
    const res = await fetch(`${process.env.GATEWAY}:${process.env.PORT}/Gojira/get_projects`);
...
ipcMain.handle('projects-update', async (e, req) => {
  try {
    ...
    const res = await fetch(`${process.env.GATEWAY}:${process.env.PORT}/Gojira/update_project`, options);
...
ipcMain.handle('projects-create', async (e, req) => {
  try {
    ...
    const res = await fetch(`${process.env.GATEWAY}:${process.env.PORT}/Gojira/create_project`, options);
