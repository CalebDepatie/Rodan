...
ipcMain.handle("pages-get", async (e, req) => {
  try {
    const res = await fetch(`${process.env.GATEWAY}:${process.env.PORT}/Gojira/get_pages`);
...
ipcMain.handle("pages-update", async (e, req) => {
  try {
    ...
    const res = await fetch(`${process.env.GATEWAY}:${process.env.PORT}/Gojira/update_page`, options);
...
ipcMain.handle("pages-create", async (e, req) => {
  try {
    ...
    const res = await fetch(`${process.env.GATEWAY}:${process.env.PORT}/Gojira/create_page`, options);
